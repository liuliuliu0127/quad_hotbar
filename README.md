...
3. **快捷键**  
   - `1`–`9` 选择左下快捷栏格子；`Ctrl` + `1`–`9` 选择右下格子。  
   - 双击数字键（或 `Ctrl` + 双击）可交换该格与上方对应格的物品。  
   - 交换键（默认 `R`）用于整行交换。  
4. **保留原双重快捷栏全部功能** – `reverseBars`、`crop`、`shift`、长按交换、双击窗口、音效等完全可用。
...
| 选择右下格子 1–9         | `Ctrl` + `1` – `9`  |
| 双击左侧单格交换         | 双击 `1` – `9`      |
| 双击右侧单格交换         | `Ctrl` + 双击 `1` – `9` |
| 滚轮切换18个底层格子     | 鼠标滚轮            |
| 修饰键（可自定义）       | 默认 `Ctrl`         |
| 只交换真实快捷栏整行     | `Ctrl` + 短按 `R`   |
| 交换背包最底层与上层     | `Ctrl` + 长按 `R`   |
...
### 配置说明
模组使用 **Cloth Config** 提供游戏内配置界面（推荐搭配 Mod Menu），你也可以手动编辑 `config/double_hotbar.json`。  
主要选项：
...
4. **All original Double Hotbar features** – `reverseBars`, `crop`, `shift`, hold‑to‑swap, double‑tap window, sound effects, etc. remain fully functional.
...
| Action                                   | Key                |
|------------------------------------------|--------------------|
| Swap active row with upper row           | Short press `R`    |
| Swap single selected slot                | Long press `R` (configurable) |
| Select left bottom slot 1–9              | `1` – `9`          |
| Select right bottom slot 1–9             | `Ctrl` + `1` – `9` |
| Double‑tap swap (left)                   | Double‑tap `1` – `9` |
| Double‑tap swap (right)                  | `Ctrl` + double‑tap `1` – `9` |
| Scroll through all 18 bottom slots       | Mouse wheel        |
| Modifier key (configurable)              | Default `Ctrl`     |
| Swap only real hotbar row                | `Ctrl` + short `R` |
| Swap bottom‑most inventory rows          | `Ctrl` + long `R`  |
...
### Configuration
The mod uses **Cloth Config** to provide an in‑game config screen (Mod Menu recommended). You can also manually edit `config/double_hotbar.json`.  
Key options:
